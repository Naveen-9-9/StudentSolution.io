...
function ActionSearchBar() {
    ...
    const fetchSuggestions = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`${apiUrl}/search/suggest?q=${encodeURIComponent(debouncedQuery)}`);
            const data = await res.json();

            if (data.success && data.data.suggestions) {
                const apiActions = data.data.suggestions.map((s: ApiSuggestion) => ({
                    id: s.id,
                    label: s.name,
                ...
                setResult({ actions: [...allActions, ...apiActions] });
            } else {
                setResult({ actions: allActions });
            }
        } catch (error) {
